...
def single_clip(location, clip_name):
    clip = VideoFileClip(location).resize(width=720).margin(top=TOP_MARGIN).afx(volumex, VOLUME)
    ...
    video.write_videofile(clip_name, preset="ultrafast", fps=30)
...
def multi_clips(clips, clip_name):
    pass
...
class TikVideo:
    def __init__(self, loc, clip_name):
        ...
        with open('info.pkl', 'rb') as f:
            self.start, self.part = pickle.load(f)
    ...
    def create_next_clip(self, clip_len):
        ...
        if self.duration < (self.start + clip_len + 20):
            clip = clip.subclip(self.start, self.duration).resize(width=720).margin(top=TOP_MARGIN).afx(volumex, VOLUME)
        else:
            clip = clip.subclip(self.start, self.start + clip_len).resize(width=720).margin(top=TOP_MARGIN).afx(volumex, VOLUME)
